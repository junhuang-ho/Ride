...
def test_setRatingBounds_require_2_revert(ride_rater):
    with brownie.reverts(
        ...
        ride_rater.setRatingBounds(3, 2)
...
def test_setRatingBounds(ride_rater):
    ...
    tx = ride_rater.setRatingBounds(2, 4)
    tx.wait(1)
...
def test_getRatingMin(ride_rater):
    assert ride_rater.getRatingMin() == 1


def test_getRatingMax(ride_rater):
    assert ride_rater.getRatingMax() == 5
