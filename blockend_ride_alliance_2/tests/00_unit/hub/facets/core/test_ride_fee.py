...
def test_setCostPerMetre(
    ...
    tx = ride_fee.setCostPerMetre(key_MYR, 3)
    tx.wait(1)
...
def test_getFare(ride_fee, ride_currency_registry, ride_access_control, deployer):
    ...
    tx = ride_fee.setCostPerMetre(key_MYR, cost_per_metres)
    tx.wait(1)

    assert ride_fee.getFare(deployer, key_MYR, minutes_taken, metres_travelled) == fare
...
def test_getCancellationFee(
    ...
    assert ride_fee.getCancellationFee(deployer, key_MYR) == 3


def test_getBaseFee(ride_fee, ride_currency_registry, ride_access_control, deployer):
    setup_currency_suppport(ride_currency_registry, ride_access_control, deployer)

    tx = ride_fee.setBaseFee(key_MYR, 3)
    tx.wait(1)

    assert ride_fee.getBaseFee(deployer, key_MYR) == 3


def test_getCostPerMinute(
    ride_fee, ride_currency_registry, ride_access_control, deployer
):
    setup_currency_suppport(ride_currency_registry, ride_access_control, deployer)
    ...
    assert ride_fee.getCostPerMinute(deployer, key_MYR) == 3


def test_getCostPerMetre(
    ride_fee, ride_currency_registry, ride_access_control, deployer
):
    setup_currency_suppport(ride_currency_registry, ride_access_control, deployer)
    ...
    assert ride_fee.getCostPerMetre(deployer, key_MYR) == 3
