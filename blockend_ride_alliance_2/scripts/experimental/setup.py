...
def deploy_hub(deployer):
    ...
    contract_ride_alliance_forger = brownie.Contract.from_abi(
        "RideAllianceForger", ride_hub.address, brownie.RideAllianceForger.abi, deployer
    )
...
def forge_alliance(
    ...
    args = (
        ...
        proposal_threshold,
        quorum_percentage,
        [driver_ally_1, driver_ally_2],
        ["firsty", "secondy"],
    )
    encoding = ride_alliance_forger.forge.encode_input(*args)

    ally_key = ride_alliance_forger.getAllianceKey(name, symbol)

    assert (
        ride_alliance_forger.getAllianceGovernanceToken(ally_key) == utils.ZERO_ADDRESS
    )
    assert ride_alliance_forger.getAllianceTimelock(ally_key) == utils.ZERO_ADDRESS
    assert ride_alliance_forger.getAllianceGovernor(ally_key) == utils.ZERO_ADDRESS
    ...
    address_alliance = ride_alliance_forger.getAllianceGovernanceToken(ally_key)
    address_alliance_timelock = ride_alliance_forger.getAllianceTimelock(ally_key)
    address_alliance_governor = ride_alliance_forger.getAllianceGovernor(ally_key)
    ...
    first_ride_ally_governance_token = brownie.Contract.from_abi(
        "RideAllianceGovernanceToken",
        address_alliance,
        brownie.RideAllianceGovernanceToken.abi,
        deployer,
    )

    first_ride_ally_timelock = brownie.Contract.from_abi(
    ...
    first_ride_ally_governor = brownie.Contract.from_abi(
...
def role_setup_alliance(
    ...
    assert not ride_access_control.hasRole(
        role_alliance, first_ride_ally_timelock.address
    )
    assert not ride_access_control.hasRole(role_alliance, deployer)

    args = (role_alliance, first_ride_ally_timelock.address)
    ...
    assert ride_access_control.hasRole(role_alliance, first_ride_ally_timelock.address)
    assert not ride_access_control.hasRole(role_alliance, deployer)
