...
contract RidePassenger {
    ...
    function startTrip(address _driver) external {
        ...
        bytes32 tixId = s2.userToTixId[msg.sender];
        s1.driverToDriverDetails[_driver].countStart += 1;
        s2.tixIdToTicket[tixId].tripStart = true;
        s2.tixIdToTicket[tixId].forceEndTimestamp =
            block.timestamp +
            s2.allianceToForceEndDelay[
                s1.driverToDriverDetails[_driver].alliance
            ];
    ...
    function endTripPax(bool _agree, uint256 _rating) external {
        ...
        if (s2.tixIdToDriverEnd[tixId].reached) {
            s1.driverToDriverDetails[driver].metresTravelled += s2
                .tixIdToTicket[tixId]
                .metres;
            s1.driverToDriverDetails[driver].countEnd += 1;
        }
    ...
    function forceEndPax() external {
        ...
        RideLibPenalty._temporaryBan(
            RideLibDriverDetails
                ._storageDriverDetails()
                .driverToDriverDetails[driver]
                .alliance,
            driver
        );
