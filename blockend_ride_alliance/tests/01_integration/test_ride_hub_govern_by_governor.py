...
def test_RideBadge_setBadgesMaxScores(
    ride_badge,
    ride_hub,
    ride_governor,
    ride_access_control,
    ...
    # 1 # --> revert as not contract owner
    with brownie.reverts():
        ride_badge.setBadgesMaxScores(badges_max_scores, {"from": deployer})

    # 2 # --> encode function to be executed
    args = (badges_max_scores,)
    encoded_function = ride_badge.setBadgesMaxScores.encode_input(*args)

    # 3 # --> passs through governance process

    # before
    assert ride_badge.getBadgeToBadgeMaxScore(0) != badges_max_scores[0]
    assert ride_badge.getBadgeToBadgeMaxScore(1) != badges_max_scores[1]
    assert ride_badge.getBadgeToBadgeMaxScore(2) != badges_max_scores[2]
    assert ride_badge.getBadgeToBadgeMaxScore(3) != badges_max_scores[3]
    assert ride_badge.getBadgeToBadgeMaxScore(4) != badges_max_scores[4]
