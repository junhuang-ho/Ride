...
def test_requireAddedXPerYPriceFeedSupported_revert(ride_exchange):
    with brownie.reverts("RideLibExchange: Price feed not supported"):
        ride_exchange.requireXPerYPriceFeedSupported_(key_ETH, key_EUR)


def test_requireAddedXPerYPriceFeedSupported_pass(
    ...
    assert ride_exchange.requireXPerYPriceFeedSupported_(key_ETH, key_EUR)


def test_requireDerivedXPerYPriceFeedSupported_revert(ride_exchange):
    with brownie.reverts("RideLibExchange: Derived price feed not supported"):
        ride_exchange.requireDerivedXPerYPriceFeedSupported_(key_ETH, key_MYR)


def test_requireDerivedXPerYPriceFeedSupported_pass(
    ride_exchange, dMockV3Aggregator1, dMockV3Aggregator2
):
    price_feed_ETH_EUR, price_feed_MYR_EUR = setup_price_feed(
        dMockV3Aggregator1, dMockV3Aggregator2
    ...
    assert ride_exchange.requireDerivedXPerYPriceFeedSupported_(key_ETH, key_MYR)
...
def test_addXPerYPriceFeed_require_1_revert(ride_exchange, ride_currency_registry):
    ...
    with brownie.reverts("RideLibExchange: Zero price feed address"):
        ride_exchange.addXPerYPriceFeed_(key_ETH, key_EUR, utils.ZERO_ADDRESS)
...
def test_addXPerYPriceFeed_require_2_revert(
    ...
    with brownie.reverts("RideLibExchange: Price feed already supported"):
        ride_exchange.addXPerYPriceFeed_(key_ETH, key_EUR, price_feed_ETH_EUR)
...
def test_deriveXPerYPriceFeed_require_1_revert(ride_exchange):
    with brownie.reverts(
        ...
        ride_exchange.deriveXPerYPriceFeed_(key_ETH, key_ETH, key_EUR)
...
def test_deriveXPerYPriceFeed_require_2_revert(
    ...
    with brownie.reverts("RideLibExchange: Derived price feed already supported"):
        ride_exchange.deriveXPerYPriceFeed_(key_ETH, key_MYR, key_EUR)
...
def test_removeDerivedXPerYPriceFeed(
    ...
    tx = ride_exchange.removeDerivedXPerYPriceFeed_(key_ETH, key_MYR)
    tx.wait(1)
...
def test_removeAddedXPerYPriceFeed_revert(
    ...
    with brownie.reverts("RideLibExchange: Base key being used"):
        ride_exchange.removeAddedXPerYPriceFeed_(key_ETH, key_EUR)
...
def test_removeAddedXPerYPriceFeed(
    ...
    tx = ride_exchange.removeAddedXPerYPriceFeed_(key_ETH, key_EUR)
    tx.wait(1)
...
def test_getAddedXPerYInWei(
    ...
    assert (
        ride_exchange.getAddedXPerYInWei_(key_ETH, key_EUR) == "2 ether"
    )  # see conftest.py setup


def test_getDerivedXPerYInWei_no_inverse(
    ride_exchange, ride_currency_registry, dMockV3Aggregator1, dMockV3Aggregator2
):
    support_ETH_EUR(ride_currency_registry)
    ...
    # assert (
    #     ride_exchange.getDerivedXPerYInWei_(key_ETH, key_MYR)
    #     == price_feed_derived  # "0.666666666666666666 ether"
    # )
    assert (
        abs(ride_exchange.getDerivedXPerYInWei_(key_ETH, key_MYR) - price_feed_derived)
        < 5000
    )


def test_getDerivedXPerYInWei_denominator_inverse(
    ride_exchange, ride_currency_registry, dMockV3Aggregator1, dMockV3Aggregator2
):
    support_ETH_EUR(ride_currency_registry)
    support_ETH_MYR(ride_currency_registry)
...
def test_getDerivedXPerYInWei_numerator_inverse(
    ride_exchange, ride_currency_registry, dMockV3Aggregator1, dMockV3Aggregator2
):
    support_ETH_EUR(ride_currency_registry)
    support_ETH_MYR(ride_currency_registry)
...
def test_getDerivedXPerYInWei_both_inverse(
    ride_exchange, ride_currency_registry, dMockV3Aggregator1, dMockV3Aggregator2
):
    support_ETH_EUR(ride_currency_registry)
    support_ETH_MYR(ride_currency_registry)
...
def test_convertDirect(ride_exchange):
    x_per_y_wei = 5 ** 18
    amount = 500
    value = (amount * (10 ** 18)) / x_per_y_wei

    assert ride_exchange.convertDirect_(x_per_y_wei, amount) == value


def test_convertInverse_(ride_exchange):
    x_per_y_wei = 5 ** 18
    amount = 500
    value = (amount * x_per_y_wei) / (10 ** 18)

    assert ride_exchange.convertInverse_(x_per_y_wei, amount) == value
...
def test_convertCurrency_added_no_inverse(
    ...
    amount = 500
    value_eth = ride_exchange.convertCurrency_(key_ETH, key_EUR, 500)
...
def test_convertCurrency_added_inverse(
    ...
    amount = 500
    value_eth = ride_exchange.convertCurrency_(key_EUR, key_ETH, 500)
...
def test_convertCurrency_derived_no_inverse_underlying_no_inverse(
    ...
    amount = 500
    value_eth = ride_exchange.convertCurrency_(key_ETH, key_MYR, 500)
...
def test_convertCurrency_derived_inverse_underlying_no_inverse(
    ...
    amount = 500
    value_eth = ride_exchange.convertCurrency_(key_MYR, key_ETH, 500)
