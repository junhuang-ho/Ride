...
library RideLibPenalty {
    ...
    struct StoragePenalty {
        mapping(address => uint256) allianceToBanDuration; // subDAO address to ban duration
    ...
    event SetBanDuration(address indexed alliance, uint256 banDuration);
    ...
    function _setBanDuration(uint256 _banDuration) internal {
        ...
        _storagePenalty().allianceToBanDuration[msg.sender] = _banDuration; // TODO: make sure msg.sender is subDAO timelock address
     ...
     */
    function _temporaryBan(address _alliance, address _user) internal {
        StoragePenalty storage s1 = _storagePenalty();
        uint256 banUntil = block.timestamp +
            s1.allianceToBanDuration[_alliance];
