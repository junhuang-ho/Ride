...
library RideLibAllianceAccessControl {
    bytes32 constant STORAGE_POSITION_ALLIANCEACCESSCONTROL =
        keccak256("ds.allianceaccesscontrol");
    ...
    struct StorageAllianceAccessControl {
        mapping(bytes32 => RoleData) roles;
    }

    function _storageAllianceAccessControl()
        internal
        pure
        returns (StorageAllianceAccessControl storage s)
    {
        bytes32 position = STORAGE_POSITION_ALLIANCEACCESSCONTROL;
        assembly {
            s.slot := position
        }
    }
    ...
    function _hasRole(bytes32 _role, address _account)
        ...
        returns (bool)
    {
        return _storageAllianceAccessControl().roles[_role].members[_account];
    }
    ...
    function _getRoleAdmin(bytes32 _role) internal view returns (bytes32) {
        return _storageAllianceAccessControl().roles[_role].adminRole;
    }
    ...
    function _setRoleAdmin(bytes32 _role, bytes32 _adminRole) internal {
        ...
        _storageAllianceAccessControl().roles[_role].adminRole = _adminRole;
    ...
    function _grantRole(bytes32 _role, address _account) internal {
        if (!_hasRole(_role, _account)) {
            _storageAllianceAccessControl().roles[_role].members[
                _account
            ] = true;
