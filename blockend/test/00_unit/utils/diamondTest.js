...
describe('DiamondTest', async function ()
{
    ...
    it('selectors should be associated to facets correctly -- multiple calls to facetAddress function', async () =>
    {
        assert.equal(
            addresses[0],
            await diamondLoupeFacet.facetAddress('0xbda30dec')
        )
        assert.equal(
            addresses[1],
    ...
    {
        const test1Facet = await ethers.getContractAt('Test1', diamondAddress[0])
    ...
    it('should replace supportsInterface function', async () =>
    {
        ...
        const testFacetAddress = test1Address //addresses[3]
        tx = await diamondCutFacet.rideCut(
            [{
    ...
    {
        const Test2Facet = await ethers.getContractFactory('Test2')
        const test2Facet = await Test2Facet.deploy()
        await test2Facet.deployed()
        addresses.push(test2Facet.address)
        test2Address = test2Facet.address
    ...
    {
        const test2Facet = await ethers.getContractAt('Test2', diamondAddress[0])
    ...
    it('remove all functions and facets accept \'diamondCut\' and \'facets\'', async () =>
    {
        ...
        selectors = removeSelectors(selectors, ['facets()', 'rideCut(tuple(address,uint8,bytes4[])[],address,bytes)'])
        tx = await diamondCutFacet.rideCut(
            [{
                facetAddress: ethers.constants.AddressZero,
                action: FacetCutAction.Remove,
            ...
            ethers.constants.AddressZero, '0x', { gasLimit: 1600000 })
        receipt = await tx.wait()
        if (!receipt.status)
        {
            throw Error(`Diamond upgrade failed: ${tx.hash}`)
        ...
        assert.sameMembers(facets[0][1], ['0xbda30dec'])
    ...
    it('add most functions and facets', async () =>
    {
        ...
        tx = await diamondCutFacet.rideCut(cut, ethers.constants.AddressZero, '0x', { gasLimit: 8000000 })
        receipt = await tx.wait()
        if (!receipt.status)
        {
            throw Error(`Diamond upgrade failed: ${tx.hash}`)
