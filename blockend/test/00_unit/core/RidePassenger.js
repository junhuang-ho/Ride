...
if (parseInt(chainId) === 31337)
{
    ...
    describe("RidePassenger", function ()
    {
        ...
        {
            it("Should revert if holding not more than request fee or fare", async function ()
            {
                var tx = await contractRideHolding.ssUserToCurrencyKeyToHolding_(accounts[0].address, keyPay, "4")
                var rcpt = await tx.wait()
                await expect(contractRidePassenger.requestTicket(keyLocal, keyPay, 0, false, 500, 500)).to.revertedWith("passenger's holding < requestFee or fare")
            })
            it("Should set ticket details", async function ()
            {
                ...
                expect((await contractRideTicket.sTixIdToTicket_(tixId_)).requestFee).to.not.equal(0)
