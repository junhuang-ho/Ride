...
def test_setup(
    ...
    # 1 # --> mint some tokens
    tx = ride_token.mint(person1, "100 ether", {"from": ride_multi_sigs[0]})
    tx.wait(1)

    assert ride_token.balanceOf(person1) == "100 ether"
    assert ride_governance_token.balanceOf(person1) == 0

    # 1.1 # --> swap to governance token for voting
    tx = ride_token.approve(
        ride_governance_token.address, "100 ether", {"from": person1}
    )
    tx.wait(1)

    tx = ride_governance_token.depositFor(person1, "100 ether", {"from": person1})
    tx.wait(1)

    assert ride_token.balanceOf(person1) == 0
    assert ride_governance_token.balanceOf(person1) == "100 ether"
    ...
    assert ride_governance_token.getVotes(person1) == 0

    tx = ride_governance_token.delegate(person1, {"from": person1})
    tx.wait(1)

    assert ride_governance_token.getVotes(person1) == "100 ether"
