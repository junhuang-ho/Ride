...
def test_setForceEndDelay(ride_ticket):
    tx = ride_ticket.setForceEndDelay_(12345)
    tx.wait(1)

    assert ride_ticket.sForceEndDelay() == 12345
...
def test_cleanUp(ride_ticket, deployer, person1):
    ...
    assert ride_ticket.sTixIdToTicket_(ticket_id)[2] == 1
    assert ride_ticket.sTixIdToTicket_(ticket_id)[3] == True
    assert ride_ticket.sTixIdToTicket_(ticket_id)[4] == 10
    assert ride_ticket.sTixIdToTicket_(ticket_id)[5] == key_MYR
    assert ride_ticket.sTixIdToTicket_(ticket_id)[6] == key_ETH
    assert ride_ticket.sTixIdToTicket_(ticket_id)[7] == 3
    assert ride_ticket.sTixIdToTicket_(ticket_id)[8] == 2
    assert ride_ticket.sTixIdToTicket_(ticket_id)[9] == True
    assert ride_ticket.sTixIdToTicket_(ticket_id)[10] == 12345
    ...
    assert ride_ticket.sTixIdToTicket_(ticket_id)[3] == False
    assert ride_ticket.sTixIdToTicket_(ticket_id)[4] == 0
    assert ride_ticket.sTixIdToTicket_(ticket_id)[5] == utils.ZERO_BYTES32
    assert ride_ticket.sTixIdToTicket_(ticket_id)[6] == utils.ZERO_BYTES32
    assert ride_ticket.sTixIdToTicket_(ticket_id)[7] == 0
