...
def test_getTixIdToTicket(ride_ticket, deployer, person1):
    ...
    assert ride_ticket.getTixIdToTicket(ticket_id)[2] == 1
    assert ride_ticket.getTixIdToTicket(ticket_id)[3] == True
    assert ride_ticket.getTixIdToTicket(ticket_id)[4] == 10
    assert ride_ticket.getTixIdToTicket(ticket_id)[5] == key_MYR
    assert ride_ticket.getTixIdToTicket(ticket_id)[6] == key_ETH
    assert ride_ticket.getTixIdToTicket(ticket_id)[7] == 3
    assert ride_ticket.getTixIdToTicket(ticket_id)[8] == 2
    assert ride_ticket.getTixIdToTicket(ticket_id)[9] == True
    assert ride_ticket.getTixIdToTicket(ticket_id)[10] == 12345
...
def test_getForceEndDelay(ride_ticket):
    tx = ride_ticket.setForceEndDelay(12345)
    tx.wait(1)

    assert ride_ticket.getForceEndDelay() == 12345
