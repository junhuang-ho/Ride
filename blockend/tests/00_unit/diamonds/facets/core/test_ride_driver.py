...
def setup_acceptTicket(ride_driver, ride_badge, ride_exchange, deployer):
    tx = ride_badge.ssDriverToDriverReputation_(
        deployer, 2, "test123", 9, 8, 7, 6, 5, 4
    )
    tx.wait(1)
...
def test_acceptTicket_require_1_revert(
    ride_driver, ride_badge, ride_exchange, deployer
):
    setup_acceptTicket(ride_driver, ride_badge, ride_exchange, deployer)

    with brownie.reverts("RideDriver: Ticket not exists"):
        ride_driver.acceptTicket(key_MYR, key_ETH, ticket_id, 0)


def test_acceptTicket_require_2_revert(
    ride_driver, ride_badge, ride_exchange, ride_ticket, deployer, person1
):
    setup_acceptTicket(ride_driver, ride_badge, ride_exchange, deployer)
...
def test_acceptTicket_require_4_revert(
    ...
    with brownie.reverts("RideDriver: Badge rank not achieved"):
        ride_driver.acceptTicket(key_MYR, key_ETH, ticket_id, 1)
...
def test_acceptTicket_require_6_revert(
    ride_driver, ride_badge, ride_exchange, ride_ticket, ride_holding, deployer, person1
):
    setup_acceptTicket(ride_driver, ride_badge, ride_exchange, deployer)
...
def test_acceptTicket_require_7_revert(
    ride_driver, ride_badge, ride_exchange, ride_ticket, ride_holding, deployer, person1
):
    setup_acceptTicket(ride_driver, ride_badge, ride_exchange, deployer)
    ...
    with brownie.reverts("RideDriver: Driver not meet badge - strict"):
        ride_driver.acceptTicket(key_MYR, key_ETH, ticket_id, 0)


def test_acceptTicket_require_8_revert(
    ride_driver, ride_badge, ride_exchange, ride_ticket, ride_holding, deployer, person1
):
    setup_acceptTicket(ride_driver, ride_badge, ride_exchange, deployer)
    ...
    with brownie.reverts("RideDriver: Driver not meet badge"):
        ride_driver.acceptTicket(key_MYR, key_ETH, ticket_id, 0)
...
def test_acceptTicket(
    ...
    tx = ride_driver.acceptTicket(key_MYR, key_ETH, ticket_id, 0)
    tx.wait(1)
...
def test_cancelPickUp_event(ride_driver, ride_ticket, ride_holding, deployer, person1):
    ...
    tx = ride_ticket.ssTixIdToTicket_(
        ticket_id, person1, deployer, 0, False, 2, key_MYR, key_ETH, 2, 2, False
    )
    tx.wait(1)
...
def test_endTripDrv_reached(ride_driver, ride_ticket, ride_holding, deployer, person1):
    ...
    tx = ride_ticket.ssTixIdToTicket_(
        ticket_id, person1, deployer, 0, False, 2, key_MYR, key_ETH, 2, 2, True
    )
    tx.wait(1)
